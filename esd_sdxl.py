...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Fine-tune an SDXL UNet to erase concepts.",
    )
    parser.add_argument(
        "--basemodel_id",
        help="HF model id for any SDXL-compatible diffusers pipeline",
        type=str,
    ...
    parser.add_argument("--guidance_scale", help="guidance scale used to sample xt", type=float, default=2)
    parser.add_argument(
        "--train_method",
        help="train method (esd-x, esd-x-strict, esd-u, esd-all). Legacy aliases still work.",
        type=str,
        required=True,
    )
    ...
    parser.add_argument(
        "--lr",
        help="learning rate. If omitted, defaults are chosen per train method.",
        type=float,
        default=None,
    )
